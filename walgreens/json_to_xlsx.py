...
def read_xlsx(name, title, key):
    try:
        with open('walgreens/info/%s_info.json' % name) as f:
            urls = json.load(f)
    ...
    for col, val in enumerate(image, start=2):
        if val['name']:
            ...
            ws.cell(row=col, column=2).value = val['brand'].strip()
            if isinstance(val['name'], str):
                ws.cell(row=col, column=3).value = val['name']
                ws.cell(row=col, column=6).value = keywords(key, val['name'])
            else:
                ws.cell(row=col, column=3).value = str(val['name'])
                ws.cell(row=col, column=6).value = keywords(key, str(val['name']))
            ws.cell(row=col, column=4).value = val['description']
            ws.cell(row=col, column=5).value = val['colour']
            if val['price']:
                ws.cell(row=col, column=7).value = val['price']
            else:
                ws.cell(row=col, column=7).value = 'Priced Per Store'
            ws.cell(row=col, column=8).value = val['product_url']
            ws.cell(row=col, column=9).value = val['image_url']
            count += 1
    try:
        wb.save("walgreens/excel/%s.xlsx" % name)
    except FileNotFoundError:
        pass

# read_xlsx('haircare', 'Haircare', 'haircare')
# read_xlsx('giftsets', 'Gift Sets', 'gift, set')
# read_xlsx('bathandbody', 'Bath And Body', 'bath, body')
# read_xlsx('fragrance', 'Fragrance', 'fragrance')
# read_xlsx('cosmetics', 'Makeup', 'makeup')
# read_xlsx('naturalandorganic', 'Natural And Oganic', 'natural, organic')
# read_xlsx('accessories', 'Accessories', 'accesories')
# read_xlsx('skincare', 'Skincare', 'skincare')
# read_xlsx('suncare', 'Sun Care', 'sun, care')
