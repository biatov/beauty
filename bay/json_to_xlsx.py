...
def read_xlsx(name, title):
    try:
        with open('bay/json/%s_items.json' % name) as f:
            urls = json.load(f)
    ...
    ws.cell(row=1, column=7).value = 'Product URL'
    ws.cell(row=1, column=8).font = Font(bold=True)
    ws.cell(row=1, column=8).value = 'Image URL'
    ...
    for col, val in enumerate(image, start=2):
        ...
        ws.cell(row=col, column=5).value = keywords(val['product_url'], val['product_name'])
        ws.cell(row=col, column=6).value = val['price']
        ws.cell(row=col, column=7).value = val['product_url']
        ws.cell(row=col, column=8).value = val['image_url']
    try:
